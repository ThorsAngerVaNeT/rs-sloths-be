...
export class LoggingInterceptor implements NestInterceptor {
  ...
  public intercept(context: ExecutionContext, call$: CallHandler): Observable<unknown> {
    ...
    this.logger.log({ message, method, params, query, body, headers }, ctx);
  ...
  private logNext(body: unknown, context: ExecutionContext): void {
    ...
    this.logger.log({ message, body }, ctx);
  }
  ...
  private logError(error: Error, context: ExecutionContext): void {
    ...
    if (error instanceof HttpException) {
      ...
      if (statusCode >= HttpStatus.INTERNAL_SERVER_ERROR) {
        this.logger.error({ method, url, body, message, error }, error.stack, ctx);
      } else {
        this.logger.warn({ method, url, error, body, message }, ctx);
      }
    } else {
      this.logger.error(
